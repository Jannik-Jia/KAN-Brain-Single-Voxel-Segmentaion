...
def advanced_feature_reduction(data, method='umap', n_components=2, labels=None, 
    ...
    # 确保数据是浮点型
    data = data.astype(np.float32)  # 使用float32以兼容GPU库
    ...
    # 如果使用了GPU，确保结果转回numpy数组
    if USE_GPU and hasattr(reduced_data, 'get'):
        try:
            import cupy
            reduced_data = cupy.asnumpy(reduced_data)
        except (ImportError, AttributeError):
            if hasattr(reduced_data, 'to_numpy'):
