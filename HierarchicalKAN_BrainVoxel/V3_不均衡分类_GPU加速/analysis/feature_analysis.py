...
def analyze_feature_group_combinations(feature_groups, labels, group_names=None, verbose=True, plot=True):
    ...
    # 检查样本数量，需要时强制降低
    if len(labels) > 5000:
        sample_ratio = min(5000/len(labels), 0.5)  # 最多采样5000个样本或50%
        if verbose:
            logger.info(f"样本数 ({len(labels)}) 过多，进行采样 (比例 {sample_ratio:.2f})")
    ...
    for i, group in enumerate(group_names):
        if verbose:
            logger.info(f"\n测试单个特征组: {group}")
        X = analysis_groups[group]
        
        # 转换数据类型为float32（适用于cuML）
        if USE_GPU and hasattr(X, 'dtype') and X.dtype != np.float32:
            X = X.astype(np.float32)
        
        # 使用随机森林评估性能
        rf = rf_base  # 使用预定义的随机森林
        cv_folds = EVALUATION['classification']['cv_folds']
        acc_scores = cross_val_score(rf, X, analysis_labels, cv=cv_folds, scoring='accuracy')
        balanced_acc_scores = cross_val_score(rf, X, analysis_labels, cv=cv_folds, scoring='balanced_accuracy')
        f1_scores = cross_val_score(rf, X, analysis_labels, cv=cv_folds, scoring='f1_weighted')
        ...
        kappa_scores = cross_val_score(rf, X, analysis_labels, cv=cv_folds, scoring=cohen_kappa_scorer)
    ...
    if len(group_names) >= 2:
        ...
        for i in range(len(group_names)):
            for j in range(i+1, len(group_names)):
                ...
                # 合并特征
                X_i = analysis_groups[group_i]
                X_j = analysis_groups[group_j]
                
                # 转换数据类型（适用于cuML）
                if USE_GPU:
                    if hasattr(X_i, 'dtype') and X_i.dtype != np.float32:
                        X_i = X_i.astype(np.float32)
                    if hasattr(X_j, 'dtype') and X_j.dtype != np.float32:
                        X_j = X_j.astype(np.float32)
            ...
            for j in range(i+1, len(group_names)):
                for k in range(j+1, len(group_names)):
                    ...
                    if USE_GPU:
                        ...
                        if hasattr(X_j, 'dtype') and X_j.dtype != np.float32:
                            X_j = X_j.astype(np.float32)
                        if hasattr(X_k, 'dtype') and X_k.dtype != np.float32:
                            X_k = X_k.astype(np.float32)
                    
                    X = np.hstack([X_i, X_j, X_k])
    ...
    if len(group_names) >= 4:
        ...
        for group in group_names:
            X_g = analysis_groups[group]
            # 转换数据类型（适用于cuML）
            if USE_GPU and hasattr(X_g, 'dtype') and X_g.dtype != np.float32:
                X_g = X_g.astype(np.float32)
            all_features.append(X_g)
...
def analyze_class_separability(feature_groups, labels, verbose=True, use_sampling=False, sample_ratio=0.1):
    ...
    # 强制采样，解决大数据集问题
    if not use_sampling and len(labels) > 5000:
        use_sampling = True
        sample_ratio = min(5000/len(labels), 0.5)  # 最多采样5000个样本或50%
        if verbose:
            logger.info(f"样本数 ({len(labels)}) 过多，启用采样 (比例 {sample_ratio:.2f})")
    ...
    if USE_GPU:
        ...
        classifiers = {
            'KNN': KNeighborsClassifier(n_neighbors=5),
            'SVM': SVC(kernel='linear', C=1, probability=True, class_weight='balanced'),  # 使用线性核加速
    ...
    for group_name, group_data in analysis_groups.items():
        ...
        for clf_name, clf in classifiers.items():
            ...
            if USE_GPU and clf_name != 'MLP':
                try:
                    if hasattr(group_data, 'dtype') and group_data.dtype != np.float32:
