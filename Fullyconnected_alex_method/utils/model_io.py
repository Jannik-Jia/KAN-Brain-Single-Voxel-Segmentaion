...
def save_model_with_architecture(model, optimizer, config, training_info, save_path, 
                               ...
                               use_old_zipfile_serialization=True, scaler=None):
    """
    保存模型并包含完整的架构和超参数信息
    ...
    if scaler is not None:
        scaler_path = os.path.splitext(save_path)[0] + '_scaler.joblib'
        joblib.dump(scaler, scaler_path)
        print(f"已保存数据标准化scaler到: {scaler_path}")
        save_dict['scaler_path'] = scaler_path
...
def load_model_with_architecture(model_path, device='cpu', strict=True, load_scaler=True):
    ...
    # 加载scaler
    scaler = None
    if load_scaler and 'scaler_path' in checkpoint:
        scaler_path = checkpoint['scaler_path']
        if os.path.exists(scaler_path):
            try:
                scaler = joblib.load(scaler_path)
                print(f"成功加载标准化scaler: {scaler_path}")
            except Exception as e:
