...
这个系统使用1D训练集（38个probanden的1D MAT文件，去除_3d后缀）进行训练，复现了Alex的网络架构，使用multidim_data前341维特征。训练采用Leave-one-out策略，预测后将softmax概率映射回3D体积，便于后续分析。

## ⚠️ 当前版本：Per-Patient Z-Score 标准化

**重要**：本版本使用 **逐患者逐通道 z-score 标准化**，与全局 StandardScaler 版本不同！

| 特性 | Per-Patient Z-Score (本版本) | Global StandardScaler |
|------|------------------------------|----------------------|
| 标准化范围 | 每个患者独立 | 所有训练患者共享 |
| 统计量计算 | 每患者每通道 | 全局所有体素 |
| 测试时 | 使用测试患者自己的统计量 | 使用训练时的全局scaler |
| experiment_id | `alex_4x4096_perpatient_zscore_test{N}_v1` | `alex_4x4096_globalstd_test{N}_v1` |
| subfamily | `baseline_4x4096_per_patient` | `baseline_4x4096` |
| normalisation.type | `per_patient_zscore` | `global_standard_scaler` |

## 系统特点

- **使用1D训练集**：直接使用multidim_data和seg_one_hot数据
- **Per-Patient Z-Score 标准化**：每个患者独立进行逐通道z-score标准化（341个通道）
...
Leave-one-out: 37个训练 + 1个测试
    ↓
训练集: multidim_data (n_voxels, 341) + seg_one_hot → labels
    ↓
【Per-Patient Z-Score】每个患者独立逐通道标准化
  - 对每个患者的每个通道（341个）计算均值和标准差
  - z-score: (x - mean) / std
  - 不跨患者共享统计量
    ↓
Alex的1D网络训练: (batch, 341) → (batch, 102)
    ↓
测试集预测: 测试患者使用自己的统计量标准化后预测
    ↓
Softmax概率 (n_test_voxels, 102)
    ↓
...
### 网络参数（固定）
- `input_dim`: 341（multidim_data前341维）
...
| 方法 | 输入 | 参数量 | 特点 |
|------|------|--------|------|
| **1D (Alex)** | (341,) | ~67M | 纯特征，无空间信息 |
| **2D Patch** | (341,3,3) | ~58K | 利用2D邻域 |
| **3D Patch** | (341,3,3,3) | ~60K | 利用3D邻域 |
...
### v1.2 - Per-Patient Z-Score 标准化版本

#### 🔄 标准化方式变更
- **Per-Patient Z-Score**：每个患者独立进行逐通道z-score标准化
- **不使用全局scaler**：测试患者使用自己的统计量，不依赖训练数据
- **experiment_id 更新**：`alex_4x4096_perpatient_zscore_test{N}_v1`
- **subfamily 更新**：`baseline_4x4096_per_patient`

#### 📝 实验记录字段更新
- `preprocessing.normalisation.type`: `"per_patient_zscore"`
- `preprocessing.normalisation.params.fit_scope`: `"per_patient"`
- `model.details.normalisation_variant`: `"per_patient"`
- `method_key`: `"alex_4x4096_perpatient_zscore"`

#### 🆚 与 Global StandardScaler 版本的对比
- Global版本：所有训练数据拟合一个scaler，测试时用训练scaler
- Per-Patient版本：每个患者独立标准化，测试患者用自己的统计量
