...
【Per-Patient Z-Score 版本】
- 标准化方式：每个患者独立进行逐通道z-score标准化
- 每个患者的每个通道（共341个）单独计算均值和标准差
- 与全局StandardScaler版本的区别：不跨患者共享统计量
"""
...
def build_experiment_dict(
    ...
    # 【Per-Patient Z-Score 版本】更新实验ID和名称
    experiment_id = f"alex_4x4096_perpatient_zscore_test{args.test_subject}_v1"

    experiment_dict = {
        "experiment_id": experiment_id,
        "method_name": "Alex 4x4096 baseline (per-patient z-score, 341-d input)",
        ...
        "model": {
            ...
            "details": {
                ...
                "normalisation_variant": "per_patient",  # 【新增】标识标准化变体
                "kan_config": None,
                "tabnet_config": None,
    ...
    notes_parts = [
        "【Per-Patient Z-Score 版本】每个患者独立进行逐通道z-score标准化，不跨患者共享统计量",
...
class Brain1D_Dataset(Dataset):
    """
    从1D MAT文件中加载数据进行训练
    【Per-Patient Z-Score 版本】- 每个患者独立标准化
    """
    ...
    def __init__(self, mat_files: List[Path], is_train: bool = True,
        ...
        Args:
            ...
            is_train: 是否为训练模式
            scaler: 【已废弃】Per-Patient模式不使用全局scaler，保留参数仅为接口兼容
        """
        self.mat_files = mat_files
        self.is_train = is_train
        self.scaler = None  # Per-Patient模式不使用全局scaler

        # 加载所有数据
        self.all_data = []
        self.all_labels = []

        print(f"加载{len(mat_files)}个被试的1D数据（Per-Patient Z-Score标准化）...")
        ...
        print(f"标准化方式: Per-Patient Z-Score (每个患者独立标准化)")

    def _load_subject(self, mat_file: Path):
        """加载单个被试的1D数据，并进行Per-Patient Z-Score标准化"""
        with h5py.File(mat_file, 'r') as f:
            ...
            # 【关键】Per-Patient Z-Score标准化：在加载时就对每个患者独立标准化
...
class TestDataset(Dataset):
    ...
    用于测试集预测的数据类
    保存3D位置信息用于映射回3D
    【Per-Patient Z-Score 版本】- 测试患者使用自己的统计量标准化
    """

    def __init__(self, mat_file_1d: Path, mat_file_3d: Path, scaler: StandardScaler = None):
        """
        Args:
        ...
        with h5py.File(mat_file_1d, 'r') as f:
            ...
            # 【关键】Per-Patient Z-Score标准化：测试患者使用自己的统计量
            self.features = per_patient_zscore(multidim_data)  # (n_voxels, 341)
        ...
        print(f"测试数据: {len(self.features)} 个体素（Per-Patient Z-Score标准化）")
...
def main():
    ...
    # 创建训练数据集
    logger.info('加载训练数据...')
    logger.info('【Per-Patient Z-Score 模式】每个患者独立进行逐通道z-score标准化')
    train_dataset = Brain1D_Dataset(
        train_files_1d,
        is_train=True,
        scaler=None  # Per-Patient模式不使用全局scaler
    )

    # Per-Patient模式不需要传递scaler，每个患者使用自己的统计量
    # scaler = train_dataset.scaler  # 【已移除】

    # 创建测试数据集（需要1D和3D文件）
    logger.info('加载测试数据...')
    test_dataset = TestDataset(test_file_1d, test_file_3d, scaler=None)  # Per-Patient模式
    ...
    if args.predict_only or args.load_model:
        ...
        logger.info('【Per-Patient Z-Score 模式】测试患者使用自己的统计量标准化')
        checkpoint = torch.load(args.load_model, map_location='cpu')
        model.load_state_dict(checkpoint['model_state_dict'])
        model = model.to(device)  # 确保模型在正确的设备上
        # scaler = checkpoint['scaler']  # 【已移除】Per-Patient模式不使用全局scaler

        # 重新创建测试数据集（Per-Patient模式：测试患者使用自己的统计量）
        test_dataset = TestDataset(test_file_1d, test_file_3d, scaler=None)
        ...
        logger.info(f'测试样本数: {len(test_dataset)}（Per-Patient Z-Score标准化）')
    ...
    if not args.predict_only:
        ...
        torch.save({
            ...
            'scaler': None,  # Per-Patient模式不使用全局scaler
            'normalisation_mode': 'per_patient_zscore',  # 【新增】记录标准化模式
            'history': history,
            'args': vars(args)
        }, model_path)
        logger.info(f'模型保存至: {model_path}')
        logger.info(f'标准化模式: Per-Patient Z-Score')
    ...
    if args.predict_only:
        ...
        logger.info('\n===== 训练完成 =====')
        logger.info(f'标准化模式: Per-Patient Z-Score（每个患者独立逐通道标准化）')
