...
class EarlyStopping:
    ...
    监控指标，如果在 patience 个 epoch 内没有超过 delta 的改善，则停止训练
    """
    def __init__(self, patience=5, delta=0.01, mode='max', verbose=True):
        """
        Args:
            patience: 容忍多少个 epoch 没有改善
            delta: 最小改善阈值，改善必须 > delta 才算有效改善
    ...
    def __call__(self, score, epoch):
        ...
        if self.best_score is None:
            ...
            if self.verbose:
                print(f"  [EarlyStopping] 初始化 best_score = {score:.4f}")
            return False

        # 计算改善量
        if self.mode == 'max':
            improvement = score - self.best_score
            is_improved = improvement > self.delta
        else:  # mode == 'min'
        ...
        if is_improved:
            # 有显著改善
            if self.verbose:
                print(f"  [EarlyStopping] 改善 {improvement:.4f} > delta({self.delta}), "
                      f"更新 best: {self.best_score:.4f} -> {score:.4f}")
            self.best_score = score
            self.best_epoch = epoch
            self.counter = 0
        else:
            ...
            self.counter += 1
            if self.verbose:
                print(f"  [EarlyStopping] 无显著改善 (改善={improvement:.4f} <= delta={self.delta}), "
                      f"patience: {self.counter}/{self.patience}")

            if self.counter >= self.patience:
                self.early_stop = True
                if self.verbose:
                    print(f"  [EarlyStopping] 触发早停! Best epoch: {self.best_epoch}, Best score: {self.best_score:.4f}")
...
def train_brain_voxel_mlp_multiclass(model, train_loader, val_loader, criterion, optimizer, device, 
    ...
    try:
        ...
        for e in range(num_epochs):
            ...
            # 计算训练集的F1分数
            if len(train_all_preds) > 0 and len(train_all_targets) > 0:
                ...
                try:
                    if config is None:
                        config = {
                            ...
                            'feature_dim': getattr(model, 'input_dim', 0),
                            'num_class': getattr(model, 'num_classes', 0),
